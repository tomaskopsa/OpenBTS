...
unsigned char* KiTable::getRand(const char *rand){
	unsigned char randUnsigned[16];
	for (int i = 0; i < 16; i++)
		randUnsigned[i] = (hextoint(rand[2 * i]) << 4) | hextoint(rand[2 * i + 1]);
	return randUnsigned;
}

void KiTable::setIMSI(const char* imsi)
{
	_imsi=imsi;
}

const char* KiTable::getIMSI(){
return _imsi;
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
